...
Use this file when Claude needs to load local skills in the monorepo. Do not scan the entire `skills/` tree by default. Pick the smallest relevant set, then read only the matching `SKILL.md` files.
...
- If the task is really inside `packages/panel`, Claude should switch to `packages/panel/.claude/skills.md`; Codex CLI should use `packages/panel/.agents/skills.md`
